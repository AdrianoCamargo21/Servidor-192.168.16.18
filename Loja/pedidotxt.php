...
if (file_exists('pedido.txt')){
    ...
    while(!feof($arq))
        for($i=0; $i<1; $i++){
            ...
            $sql="select ccodproduto from aprodutos where crefporduto = '$linha[0]' and clinproduto = 108 ";
            $exsql=pg_query($con,$sql);
            $rssql=pg_fetch_array($exsql);
            $codigo=$rssql['ccodproduto'];
            if ($codigo == null) {
                echo "<script>alert('Erro Ao Localizar o Produto $linha[0] ');</script>";
                exit($sql);
            }
            $sql = "INSERT INTO $tabela (codigo,quantd) VALUES ('$codigo', '$linha[1]')";
