...
if ($tipo == C) {
    $nf = $_POST['nfe'];
    if ($nf == null) {
        echo "<script>alert('Nota Inválida');</script>";
    } else {
        $chave = $_POST['chave'];
        if(strlen($chave)> 44 or strlen($forcnpj)< 44){
            echo "<script>alert('Chave inválida');</script>";        
        } else {
            $sql = "select *from notasentrada where chave = $chave ";
            $exsql= pg_query($conc,$sql);
            $resulsql= pg_fetch_array($exsql);
            $id = $resulsql['id'];
            if ($id <> null) {
                echo "<script>alert('Nota Já Cadastrada');</script>"; 
            } else {
                $sql = "INSERT INTO notasentrada  VALUES (nextval('seq_notasentrada'),$chave,'P','$dia','$login',null)";
                $exsql= pg_query($conc,$sql);
            }
        }
    }
...
echo "<tr><td><font color=\"black\"><strong>Nfe</strong></font></td>"."<td><font color=\"black\"><strong>Cadastro</strong></font></td>".
"<td><font color=\"black\"><strong>Status</strong></font></td>"."<td><font color=\"black\"><strong>Dt Faturamento</strong></font></td>"."</tr>";
$filtro = $_POST['filtro'];
if ($filro == 'P') {
    $sql = "ddd";
} elseif ($filtro == 'F'){
    ...
    if ($login == 'ADRIANO' or $login =='TAINA' or $login =='ADRIELI') {
        $parametro = null;
    } else {
        $parametro = " where loja = '$login'";
    }
    $sql = "select *from notasentrada $parametro ";exit($sql);
}
