...
<html>
<head>
<title>Replicação de Lages</title>
...
$time=date('H:i:s');
$dia= date('d-m-Y');
if(!@($destino=pg_connect ("host=127.0.0.1 dbname=trolll port=5435 user=postgres password=ky$14gr@"))){
    ...
    <html>
    <head>
    <meta http-equiv='refresh' content='2' url=http://localhost/replica/lages.php';'>
...
$sql="select *from logrep where codigo > 4332 and automatico is null order by codigo limit 100 ";
...
if ($rsql == null) {
	...
	<html>
	<head>
	<meta http-equiv='refresh' content='20' url=http://localhost/replica/lages.php';'>	
	</head>
	</html>
	<?php header("Content-Type: text/html; charset=ISO-8859-1",true);?>
	<?php
...
while($dados=pg_fetch_array($exsql)){
    ...
    $linha ++;
    echo $linha.'º Replicado o Código:'.$codigo.' Da Data:'.$data.' E Hora:'.$time.'<br>';
}
...
<html>
<head>
<meta http-equiv='refresh' content='5' url=http://localhost/Replica/lages.php';'>
</head>
</html>