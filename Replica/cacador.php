...
<html>
<head>
<title>Replicação de Caçador</title>
...
$time=date('H:i:s');
$dia= date('d-m-Y');
if(!@($destino=pg_connect ("host=127.0.0.1 dbname=trollc port=5435 user=postgres password=ky$14gr@"))){
    ...
    <html>
    <head>
    <meta http-equiv='refresh' content='2' url=http://localhost/replica/cacador.php';'>
...
if(!@($origem=pg_connect ("host=192.168.16.190 dbname=troll_cdr port=5430 user=postgres password=ky$14gr@"))){
...
$sql="select *from logrep where codigo > 10930 and automatico is null order by codigo limit 10 ";
...
if ($rsql == null) {
	...
	<html>
	<head>
	<meta http-equiv='refresh' content='20' url=http://localhost/replica/cacador.php';'>	
	</head>
	</html>
	<?php header("Content-Type: text/html; charset=ISO-8859-1",true);?>
	<?php
...
<html>
<head>
<meta http-equiv='refresh' content='1' url=http://localhost/Replica/cacador.php';'>
</head>
</html>