...
<html>
<head>
<title>Replicador Produtos Videira</title>
...
if(!@($destino=pg_connect ("host=192.168.9.10 dbname=troll port=5430 user=postgres password=ky$14gr@"))){
    ...
    echo "<p style=background:#D3D3D3; align=center <br/><b><font size=5 color=#FF0000>Servidor Replicado de Videira Conectado</font></b></p>";
}
...
$sql="select codigo from replicadorproduto where base=1 order by codigo desc limit 1 ";
echo $sql.'<br>';
...
$sql = "delete from log2"; echo $sql.'<br>';
...
$sql="insert into log2 select *from log where tabela = 'aprodutos'  and operacao='UPDATE' and depois not like '%cqtd%' and codigo > $cod ";
echo $sql.'<br>';
$exsql=pg_query($con,$sql);
if (!$exsql){
...
$sql="insert into log2 select *from log where tabela = 'aprodutos' and operacao in ('INSERT','DELETE')  and codigo > $cod ";
echo $sql.'<br>';
$exsql=pg_query($con,$sql);
if (!$exsql){
...
$sql="insert into log2 select *from log where codigo > $cod and tabela in ('tmarca','tlinha','tgrupo','tunidadeproduto','tdepartamento','ncm','grade',
...
$sql="select *from log2 where codigo > $cod order by codigo";
echo $sql.'<br>';
$exsql=pg_query($con,$sql);
if (!$exsql){
...
while($dados=pg_fetch_array($exsql)){
    ...
    if (!$excom) {
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>Erro Ao Copiar O Cógigo=$codigo</font></b></p>";
        pg_close($serv);
        pg_close($con);
        exit;
    }
    $com="SELECT adilog( codigo , datal , hora , tabela , operacao , antes , depois , usuario , 0 ) from tplog where codigo = $codigo order by codigo";
    echo $com.'<br>';
    $excom=pg_query($destino,$com);
    if (!$excom) {        
        $com="delete from tplog where codigo = $codigo";
        $excom=pg_query($destino,$com);
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>Erro Ao Replicar O Cógigo=$codigo</font></b></p>";
        pg_close($serv);
        pg_close($con);
        exit;
    }
    $com="delete from replicadorproduto where codigo < $codigo and base = 1 ";
    echo $com.'<br>';
    ...
    $com="insert into replicadorproduto(codigo,base) VALUES ($codigo,1)";
    echo $com.'<br>';
    $excom=pg_query($serv,$com);
    if (!$excom) {
    ...
    $com="delete from log2 where codigo = $codigo";
    echo $com.'<br>';
    $excom=pg_query($con,$com);
	if (!$excom) {
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>Erro ao Apagar O Código:$codigo da Tabela log2 De Origem</font></b></p>";
        pg_close($serv);
        pg_close($con);
        exit;
    }
