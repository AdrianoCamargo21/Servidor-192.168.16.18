...
if(!@($conc=pg_connect ("host=192.168.16.2 dbname=troll port=5430 user=postgres password=ky$14gr@"))){
	...
	<meta http-equiv='refresh' content='10' ;url=http://192.168.13.2/Desenvolver/preco.php';'>
	<?php header("Content-Type: text/html; charset=ISO-8859-1",true);?>
	<?php
    exit; 
} else {
...
$sql="select ccodproduto from aprodutos where cpveproduto = cpcuproduto and ccodproduto >= 234019  order by ccodproduto desc";
...
if ($rsql == '') {
    ...
    <html>
    <head>
    <meta http-equiv='refresh' content='5' ;url=http://192.168.13.2/Desenvolver/preco.php';'>
    ...
    while($dados=pg_fetch_array($exsql)){
        ...
        $com="select cpveproduto,cpcuproduto,cpcoproduto from aprodutos  where ccodproduto = '$cod'";
        $excom=pg_query($conv,$com);
        $rscom=pg_fetch_array($excom);
        $preco=$rscom['cpveproduto'];
        $custo=$rscom['cpcuproduto'];
        $compra=$rscom['cpcoproduto'];
        if ($custo <> $preco) {
            $total ++;
            $com="select logar('COPIA',1,0); update aprodutos set cpveproduto= $preco , cpcuproduto=$custo,cpcoproduto = $compra  where ccodproduto = '$cod' ";
