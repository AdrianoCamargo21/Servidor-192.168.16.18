...
<html>
<head>
<title>Replicador Base Videira</title>
...
if(!@($destino=pg_connect ("host=127.0.0.1 dbname=troll_lv port=5435 user=postgres password=ky$14gr@"))){
    echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Sem Comunicação Banco de Dados Replicado De Videira Data:$dia  Hora:$time </font></b></p>";
    echo "<p style=background:#000000; align=center <br/><b><font size=10 color=#7CFC00>Caso Persista Favor avisar o Adriano</font></b></p>";
    pg_close($serv);
    ?>
    <!DOCTYPE html>
...
if(!@($con=pg_connect ("host=192.168.9.10 dbname=troll port=5433 user=postgres password=ky$14gr@"))){
    ...
    echo "<p style=background:#000000; align=center <br/><b><font size=10 color=#7CFC00>Caso Persista Favor avisar o Adriano</font></b></p>";
    pg_close($serv);pg_close($destino); 
    ?>
    <!DOCTYPE html>
    <html>
    <head>
...
$sql="select codigo from replicadorbase where base=2 order by codigo desc limit 1 ";
...
if (!$exsql){   
    ...
    pg_close($serv);pg_close($con);pg_close($destino);      
    exit;
}
...
$sql="select *from log where codigo > $cod and tabela <> 'versao_bd' order by codigo limit 10  ";
...
while($dados=pg_fetch_array($exsql)){
    ...
    $com="insert into temporariolog values($codigo,'$data','$hora','$operacao','$tabela','$usuario','$antes','$depois',$codusiario,NULL,NULL)";
    ...
    $com="SELECT adiclog( codigo , datal , hora , tabela , operacao , antes , depois , usuario , 0 ) from temporariolog where codigo = $codigo";
    $excom=pg_query($destino,$com);
    if (!$excom) {        
        $com="delete from temporariolog where codigo = $codigo";
        $excom=pg_query($destino,$com);
    ...
    $com="update replicadorbase set codigo = $codigo where base = 2";    
