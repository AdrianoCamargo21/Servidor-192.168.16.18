...
if ($base==1) {
    ...
    if(!@($serv=pg_connect('host=192.168.16.190 dbname=troll_cdr port=5430 user=postgres password=ky$14gr@'))){
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Sem Comunicação Banco de Dados de Caçador Data:$dia  Hora:$time </font></b></p>";
        echo "<p style=background:#000000; align=center <br/><b><font size=10 color=#7CFC00>Caso Persista Favor avisar o Adriano</font></b></p>";
        ?>
        <!DOCTYPE html>
...
if ($base==3) {    
    ...
    if(!@($serv=pg_connect('host=192.168.16.190 dbname=troll_videira port=5430 user=postgres password=ky$14gr@'))){
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Sem Comunicação Banco de Dados de Videira Data:$dia  Hora:$time </font></b></p>";
        echo "<p style=background:#000000; align=center <br/><b><font size=10 color=#7CFC00>Caso Persista Favor avisar o Adriano</font></b></p>";
        ?>
        <!DOCTYPE html>
...
if ($base==4) {
    if(!@($con=pg_connect('host=192.168.16.3 dbname=troll port=5434 user=postgres password=ky$14gr@'))){
    ...
    if(!@($serv=pg_connect('host=192.168.16.190 dbname=troll_joinville port=5430 user=postgres password=ky$14gr@'))){
        echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Sem Comunicação Banco de Dados de Joinville Data:$dia  Hora:$time </font></b></p>";
        echo "<p style=background:#000000; align=center <br/><b><font size=10 color=#7CFC00>Caso Persista Favor avisar o Adriano</font></b></p>";
        ?>
        <!DOCTYPE html>
...
$com="select cprohistorico,centhistorico,cvbrhistorico  from ahistorico where cienhistorico =$ide ";
...
if ($produto=='') {
   ...
   $excom=pg_query($con,$com);
   echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Id da Nfe de Entrada Inválido</font></b></p>";
   ?>
    <!DOCTYPE html>
	<html>
	<head>
...
while($dados=pg_fetch_array($excom)){
    ...
    $qtd=$dados['centhistorico'];
    $prec=$dados['cvbrhistorico'];
