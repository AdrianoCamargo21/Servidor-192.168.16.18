...
<?php
$volta="<script>window.location='http://192.168.13.2:8080/Desenvolver/loto.html';</script>";
set_time_limit(0);
date_default_timezone_set('America/Sao_Paulo');
$time=date('H:i:s');
$dia= date('d-m-Y');
if(!@($serv=pg_connect('host=192.168.16.190 dbname=viabrasil port=5430 user=postgres password=ky$14gr@'))){
    echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! Sem Comunicação Banco de Dados  Data:$dia  Hora:$time </font></b></p>";
...
while(!feof($arq))
    for($i=0; $i<1; $i++){
		...
		while (1 == 1){
			$l = $var;
			if ($linha[$l] <> null ) { 
				$sql = "select *from resultado where numero = $linha[$l] "; 
				$exsql=pg_query($serv,$sql) ;
				$rssql= pg_fetch_array($exsql);
				$qtd=$rssql['vezes'];
				if ($qtd == null) {
					$sql = "INSERT INTO resultado VALUES ($linha[$l] , 1)"; 
					$result = pg_query($sql) ;
				} else {
					$qtd += 1 ;
					$sql = "update resultado set vezes = $qtd where numero = $linha[$l] ";
					$result = pg_query($sql) ;
				}
    ...
    for($i=0; $i<1; $i++){
        ...
        if ($linha[0] > 1 ) {
            ...
            $linha = array();// linpa o array de $linha e volta para o for            
				...
				echo "<script>alert('Erro Na Linha $ll');</script>";            	
            }
    }
    echo "<script>alert('Coleta 01 Carregada Com Sucesso');</script>";
    pg_close($serv);
    echo $volta;
    exit;
}
if ($tipo == '2'){
    ...
    echo "<script>alert('Coleta 02 Carregada Com Sucesso');</script>";
    pg_close($serv);
    echo $volta;
    exit;
}
if ($tipo == '3'){
    ...
    echo "<script>alert('Coleta 03 Carregada Com Sucesso');</script>";
    pg_close($serv);
    echo $volta;
    exit;
}
