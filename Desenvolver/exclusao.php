...
$com="select saida  from adrianoidbomjesus where saida  = $id ";
$excom=pg_query($con,$com);
$rscom=pg_fetch_array($excom);
$saida=$rscom['saida'];
if ($saida  <> '') {
    echo "<p style=background:#000000; align=center <br/><b><font size=30 color=#FF0000>ERRO!!! **Id ja dado baixa**</font></b></p>";
    ?>
    <!DOCTYPE html>
	<html>
	<head>
    ...
    $com="INSERT INTO adrianoidbomjesus(entrada, saida,preco)VALUES (0,$id,'$valor')";
    $excom=pg_query($con,$com);
}

$com="select cprohistorico,csaihisotorico from ahistorico where cisahistorico = $id order by cprohistorico ";
set_time_limit(900);
$excom=pg_query($con,$com);
$rscom=pg_fetch_array($excom);
...
while($dados=pg_fetch_array($excom)){
    ...
    if ($qnta == '') {
        ...
        $qntn=$qnta-$qnt;
        if ($qntn < '0') {
            $sqltri="update $tabela set quantidade = '0' where codigo = $codigo ";
            pg_query($con,$sqltri);
            $qntn= $qnt-$qnta;
            echo "Produdo:$codigo com divergência nas devoluções Quantidade Divergente: $qntn".'<br>' ;        
        } else {
...
<center><img src="img/saida.png"alt="1" heigth ="500px" width="500px" ></center>
</head>
</html>
