...
<html>
<head>
<title>Replicador Clientes Lages</title>
...
if(!@($serv = pg_connect ("host = 127.0.0.1 dbname = via_brasil port = 5430 user = postgres password = ky$14gr@"))){
    ...
    <html>
    <head>
    <meta http-equiv='refresh' content='5' url=http://localhost/Desenvolver/clientesl.php';'>
...
if(!@($destino = pg_connect ("host = 192.168.11.2 dbname = troll port = 5430 user = postgres password = ky$14gr@"))){
...
$sql = "select codigo from replicadorcliente where base = 3 order by codigo desc limit 1 ";echo $sql.'<br>';
...
$sql = "select *from log where codigo > $cod and tabela in ('aclientes','tcliente','tprofissao','tclasse_cliente','tregiao','tcep') order by codigo limit 10 ";echo $sql.'<br>';//
...
while($dados=pg_fetch_array($exsql)){
    ...
    $com = "SELECT adilog( codigo , datal , hora , tabela , operacao , antes , depois , usuario , 0 ) from tplog where codigo = $codigo";echo $com.'<br>';
    $excom = pg_query($destino,$com);
    if (!$excom) {        
    ...
    $com = "insert into replicadorcliente(codigo,base) VALUES ($codigo,3)";echo $com.'<br>';
    ...
    $com = "delete from replicadorcliente where codigo < $codigo and base = 3 ";
    $excom=pg_query($serv,$com);
    if (!$excom) {
