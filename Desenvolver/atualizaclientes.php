...
$sql="select ccodigo,ccpf from aclientes where ccpf > 0  and ccodigo <> 1   and ccodigo <> 136962 order by ccodigo ";
...
$com="select ccodigo from aclientes where ccpf='$cpf' ";
    ...
    $codn=$rscom['ccodigo'];
    if ($codn == '') {    
        echo "Cliente $cod Não Cadastrado Em Caçador".'<br>';
    } else {
        $com="update asaidas set cclisaidas = $codn where  cclisaidas = $cod ";
        $excom=pg_query($conj,$com);
        $com="update ahcontas set cod_cli_his = $codn where  cod_cli_his = $cod";
        $excom=pg_query($conj,$com);        
        $com="update ahistorico set cclihistorico = $codn  where cclihistorico=$cod ";
        echo $com."<br>";
        $com="update asduplicatas set cclidupli = $codn  where cclidupli=$cod ";
        $excom=pg_query($conj,$com); 
        
    }
}
$com="update asaidas set cclisaidas = 13962 where  cclisaidas = 1";
$excom=pg_query($conj,$com);
$com="update ahcontas set cod_cli_his = 13962 where cod_cli_his  = 1 ";
$excom=pg_query($conj,$com);
$com="update ahistorico set cclihistorico = 13962  where cclihistorico=1 ";
echo $com."<br>";
$com="update asduplicatas set cclidupli = 1  where cclidupli=13962 ";
$excom=pg_query($conj,$com); 
