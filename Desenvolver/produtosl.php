...
<html>
<head>
<title>Replicador Produtos Lages</title>
...
$sql="select codigo from replicadorproduto where base=3 order by codigo desc limit 1 ";
...
$sql="insert into log4 select *from log where tabela = 'aprodutos' and operacao='UPDATE' and depois not like '%cqtd%' and codigo > $cod ";
$exsql=pg_query($con,$sql);
if (!$exsql){
	$sql="delete from log4 ";
	$exsql=pg_query($con,$sql);
...
$sql="insert into log4 select *from log where tabela = 'aprodutos' and operacao in ('INSERT','DELETE')  and codigo > $cod and usuario <> 'COPIA' ";
$exsql=pg_query($con,$sql);
if (!$exsql){
...
$sql="insert into log4 select *from log where codigo > $cod and tabela in ('tmarca','tlinha','tgrupo','tunidadeproduto','tdepartamento','ncm','grade',
...
$sql="select *from log4 where codigo > $cod order by codigo";
$exsql=pg_query($con,$sql);
if (!$exsql){
...
while($dados=pg_fetch_array($exsql)){
    ...
    $com="delete from replicadorproduto where codigo < $codigo and base = 3 ";
    ...
    $com="insert into replicadorproduto(codigo,base) VALUES ($codigo,3)";
    $excom=pg_query($serv,$com);
    if (!$excom) {
    ...
    $com="delete from log4 where codigo = $codigo";
    $excom=pg_query($con,$com);
	if (!$excom) {
